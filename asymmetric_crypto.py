...
def save_keys(private_key, public_key, public_path: str, private_path: str):
    """
    ...
    try:
        ...
        save_bytes_to_file(private_pem, private_path)
        ...
        save_bytes_to_file(public_pem, public_path)
    except Exception as e:
...
def encrypt_symmetric_key(symmetric_key: bytes, public_key) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(f"Ошибка шифрования ключа: {str(e)}")
    except Exception as e:
...
def decrypt_symmetric_key(encrypted_key: bytes, private_key) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(f"Ошибка дешифрования ключа: {str(e)}")
    except Exception as e:
        raise RuntimeError(f"Неожиданная ошибка: {str(e)}")